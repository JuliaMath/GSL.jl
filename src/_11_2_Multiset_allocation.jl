...
function multiset_init_first()
    c = Ref{gsl_multiset}()
    ccall( (:gsl_multiset_init_first, libgsl), Nothing, (Ref{gsl_multiset},
        ), c )
    return c[]
end
...
function multiset_init_last()
    c = Ref{gsl_multiset}()
    ccall( (:gsl_multiset_init_last, libgsl), Nothing, (Ref{gsl_multiset}, ),
        c )
    return c[]
end
...
function multiset_free(c::Ref{gsl_multiset})
    ccall( (:gsl_multiset_free, libgsl), Nothing, (Ref{gsl_multiset}, ), c )
end
