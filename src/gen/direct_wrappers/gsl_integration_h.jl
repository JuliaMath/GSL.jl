...
> This function allocates space for a gsl\_integration\_qaws\_table
...
> gsl\_integration\_qaws\_table if no errors were detected, and 0 in the
> case of error.
...
> an existing gsl\_integration\_qaws\_table struct t.
...
> gsl\_integration\_qaws\_table struct t.
...
> This function allocates space for a gsl\_integration\_qawo\_table
...
> is made with the parameter sine which should be chosen from one of the
> two following symbolic values:
>
> The gsl\_integration\_qawo\_table is a table of the trigonometric
...
> Each level corresponds to one bisection of the interval $L$, so that n
> levels are sufficient for subintervals down to the length $L/2^n$. The
> integration routine gsl\_integration\_qawo returns the error
> `GSL_ETABLE`{.sourceCode} if the number of levels is insufficient for
...
> final approximation, result, an estimate of the absolute error, abserr
> and the number of function evaluations used, neval. The Gauss-Kronrod
> rules are designed in such a way that each rule uses all the results
...
> absolute and relative error limits, epsabs and epsrel. The function
> returns the final approximation, result, and an estimate of the
> absolute error, abserr. The integration rule is determined by the
> value of key, which should be chosen from the following symbolic
> names,
>
> -   -   -   -   -   -   
>
...
> discontinuities and integrable singularities. The function returns the
> final approximation from the extrapolation, result, and an estimate of
...
> region then this routine will be faster than gsl\_integration\_qags.
...
> absolute error, abserr. The subintervals and their results are stored
> in the memory provided by workspace. The maximum number of
> subintervals is given by limit, which may not exceed the allocated
> size of the workspace.
>
...
> the table wf (the length L can take any value, since it is overridden
> by this function to a value appropriate for the Fourier integration).
> The integral is computed using the QAWO algorithm over each of the
> subintervals,
>
...
> by workspace. The maximum number of subintervals is given by limit,
> which may not exceed the allocated size of the workspace. The
> integration over each subinterval uses the memory provided by
> cycle\_workspace as workspace for the QAWO algorithm.
...
> Gauss-Legendre point xi and weight wi on the interval \[a, b\]. The
> points and weights are ordered by increasing point value. A function
> $f$ may be integrated on \[a, b\] by summing $wi * f(xi)$ over i.
...
> intervals. The number n is not the maximum number of intervals that
> will be evaluated. If the workspace is full, intervals with smaller
> error estimates will be discarded. A minimum of 3 intervals is
...
> desired absolute and relative error limits, epsabs and epsrel using
> the |cquad| algorithm. The function returns the final approximation,
> result, an estimate of the absolute error, abserr, and the number of
> function evaluations required, nevals.
>
...
> evaluated and an error estimate is computed based on the $L_2$-norm of
> the difference between the underlying interpolating polynomials of
> both rules. If the highest-degree rule has already been used, or the
> interpolatory polynomials differ significantly, the interval is
> bisected.
>
> The subintervals and their results are stored in the memory provided
> by workspace. If the error estimate or the number of function
> evaluations is not needed, the pointers abserr and nevals can be set
> to `NULL`{.sourceCode}.
...
> This function integrates $f(x)$, specified by f, from a to b, storing
> the answer in result. At each step in the iteration, convergence is
> tested by checking:
>
...
> stores the best current estimate in result and returns GSL\_EMAXITER.
> If the method converges, the function returns GSL\_SUCCESS. The total
> number of function evaluations is returned in neval.
...
> interpolating quadratures using n quadrature nodes. The parameters a,
> b, alpha, and beta specify the integration interval and/or weighting
> function for the various quadrature types. See the
> table &lt;tab\_fixed-quadratures&gt; above for constraints on these
