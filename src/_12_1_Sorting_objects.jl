...
# 
#   Returns: Nothing
function heapsort(array::Ref{Nothing}, count::Integer, size::Integer, compare::gsl_comparison_fn_t)
    ccall( (:gsl_heapsort, libgsl), Nothing, (Ref{Nothing}, Csize_t, Csize_t,
...
# 
#   Returns: Cint
function heapsort_index(array::Ref{Nothing}, count::Integer, size::Integer, compare::gsl_comparison_fn_t)
    p = Ref{Csize_t}()
    errno = ccall( (:gsl_heapsort_index, libgsl), Cint, (Ref{Csize_t},
        Ref{Nothing}, Csize_t, Csize_t, gsl_comparison_fn_t), p, array, count,
        size, compare )
