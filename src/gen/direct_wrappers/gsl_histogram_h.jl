...
> returns a pointer to a newly created gsl\_histogram struct. If
...
> the histogram then the function returns GSL\_EDOM, and none of bins
> are modified. Similarly, if the value of x is greater than or equal to
> the upper limit of the histogram then the function returns GSL\_EDOM,
> and none of the bins are modified. The error handler is not called,
...
> This function is similar to gsl\_histogram\_increment but increases
...
> sets the index i and returns GSL\_SUCCESS. If x lies outside the valid
> range of the histogram then the function returns GSL\_EDOM and the
...
> If i lies outside the valid range of indices for the histogram then
> the error handler is called with an error code of GSL\_EDOM and the
> function returns 0.
...
> of determining these values without accessing the gsl\_histogram
> struct directly.
...
> array range of size size. The values of the histogram bins are reset
> to zero. The range array should contain the desired bin limits. The
...
> range xmin to xmax uniformly. The values of the histogram bins are
> reset to zero. The bin ranges are shown in the table below,
>
...
> This function reads into the histogram h from the open stream stream
> in binary format. The histogram h must be preallocated with the
> correct size since the function uses the number of bins in h to
...
> output is formatted in three columns, and the columns are separated by
> spaces, like this:
>
...
>
> The values of the ranges are formatted using range\_format and the
> value of the bins are formatted using bin\_format. Each line contains
...
> by gsl\_histogram\_fprintf. The histogram h must be preallocated with
> the correct length since the function uses the size of h to determine
...
> bins and returns a pointer to a newly initialized gsl\_histogram\_pdf
...
> contents of the histogram h. If any of the bins of h are negative then
> the error handler is invoked with an error code of GSL\_EDOM because a
...
> This function frees the probability distribution function p and all of
...
> compute a single random sample from the probability distribution p.
> The algorithm used to compute the sample $s$ is given by the following
> formula,
>
