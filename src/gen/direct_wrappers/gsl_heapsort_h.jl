...
> size size, into ascending order using the comparison function compare.
> The type of the comparison function is defined by
>
> A comparison function should return a negative integer if the first
> argument is less than the second argument, `0`{.sourceCode} if the two
> arguments are equal and a positive integer if the first argument is
...
> This function indirectly sorts the count elements of the array array,
> each of size size, into ascending order using the comparison function
> compare. The resulting permutation is stored in p, an array of length
> n. The elements of p give the index of the array element which would
> have been stored in that position if the array had been sorted in
> place. The first element of p gives the index of the least element in
> array, and the last element of p gives the index of the greatest
