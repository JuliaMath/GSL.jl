...
############################
export vector_isnull, vector_ispos, vector_isneg, vector_isnonneg, vector_equal
...
function vector_isnull(v::Ptr{gsl_vector})
    errno = ccall( (:gsl_vector_isnull, :libgsl), Cint, (Ptr{gsl_vector},
        ), v )
...
function vector_ispos(v::Ptr{gsl_vector})
    errno = ccall( (:gsl_vector_ispos, :libgsl), Cint, (Ptr{gsl_vector}, ),
        v )
    if errno!= 0 throw(GSL_ERROR(errno)) end
end
...
function vector_isneg(v::Ptr{gsl_vector})
    errno = ccall( (:gsl_vector_isneg, :libgsl), Cint, (Ptr{gsl_vector}, ),
        v )
    if errno!= 0 throw(GSL_ERROR(errno)) end
end
...
function vector_isnonneg(v::Ptr{gsl_vector})
    errno = ccall( (:gsl_vector_isnonneg, :libgsl), Cint, (Ptr{gsl_vector},
        ), v )
    if errno!= 0 throw(GSL_ERROR(errno)) end
end
...
function vector_equal(u::Ptr{gsl_vector}, v::Ptr{gsl_vector})
    errno = ccall( (:gsl_vector_equal, :libgsl), Cint, (Ptr{gsl_vector},
        Ptr{gsl_vector}), u, v )
    if errno!= 0 throw(GSL_ERROR(errno)) end
end
