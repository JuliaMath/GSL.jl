...
>
> -   For the Cholesky solver, the matrix $J^T J$ is factored at
>     each iteration. Therefore this function will estimate the 1-norm
>     condition number
...
>     $rcond = \sigma_{min} / \sigma_{max}$. Note that when using |More|
...
>     dimensionless quantity and so independent of the scale of
>     the problem. The `max`{.sourceCode} arguments help ensure the test
