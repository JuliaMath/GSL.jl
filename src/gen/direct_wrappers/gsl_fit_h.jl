...
> vectors of length n with strides xstride and ystride. The errors on y
> are assumed unknown so the variance-covariance matrix for the
> parameters (c0, c1) is estimated from the scatter of the points around
> the best-fit line and returned via the parameters (cov00, cov01,
> cov11). The sum of squares of the residuals from the best-fit line is
> returned in sumsq. Note: the correlation coefficient of the data can
> be computed using gsl\_stats\_correlation, it does not depend on the
> fit.
...
> vector w, of length n and stride wstride, specifies the weight of each
...
> This function uses the best-fit linear regression coefficients c0, c1
> and their covariance cov00, cov01, cov11 to compute the fitted
> function y and its standard deviation y\_err for the model
...
> unknown so the variance of the parameter c1 is estimated from the
> scatter of the points around the best-fit line and returned via the
> parameter cov11. The sum of squares of the residuals from the best-fit
...
> This function computes the best-fit linear regression coefficient c1
> of the model $Y = c_1 X$ for the weighted datasets (x, y), two vectors
> of length n with strides xstride and ystride. The vector w, of length
...
> its covariance cov11 to compute the fitted function y and its standard
> deviation y\_err for the model $Y = c_1 X$ at the point x.
