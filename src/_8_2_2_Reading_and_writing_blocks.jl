...
# 
#   Returns: Cint
function block_fwrite(stream::Ref{Nothing}, b::Ref{gsl_block})
    errno = ccall( (:gsl_block_fwrite, libgsl), Cint, (Ref{Nothing},
...
# 
#   Returns: Cint
function block_fread(stream::Ref{Nothing})
    b = Ref{gsl_block}()
    errno = ccall( (:gsl_block_fread, libgsl), Cint, (Ref{Nothing},
        Ref{gsl_block}), stream, b )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return b[]
end
...
# 
#   Returns: Cint
function block_fprintf(stream::Ref{Nothing}, b::Ref{gsl_block})
    format = Ref{Cchar}()
    errno = ccall( (:gsl_block_fprintf, libgsl), Cint, (Ref{Nothing},
...
# 
#   Returns: Cint
function block_fscanf(stream::Ref{Nothing})
    b = Ref{gsl_block}()
    errno = ccall( (:gsl_block_fscanf, libgsl), Cint, (Ref{Nothing},
        Ref{gsl_block}), stream, b )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return b[]
end
