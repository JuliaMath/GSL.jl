...
> abserr. The result and its error estimate are based on a weighted
> average of independent samples. The chi-squared per degree of freedom
> for the weighted average is returned via the state struct component,
> `s->chisq`{.sourceCode}, and must be consistent with 1 for the
...
integral internally, according to the `iterations`{.sourceCode}
parameter described below, and returns their weighted average. Random
sampling of the integrand can occasionally produce an estimate where the
error is zero, particularly if the function is constant in some regions.
An estimate with zero error causes the weighted average to break down
and must be handled separately. In the original Fortran implementations
of VEGAS the error estimate is made non-zero by substituting a small
value (typically `1e-30`{.sourceCode}). The implementation in GSL
differs from this and avoids the use of an arbitrary constant---it
either assigns the value a weight which is the average weight of the
preceding estimates or discards it according to the following procedure,
...
gsl\_monte\_vegas\_params\_get, the necessary changes are made to the
fields of the params structure, and the values are copied back into the
integrator state using gsl\_monte\_vegas\_params\_set. The functions use
the gsl\_monte\_vegas\_params structure which contains the following
fields:
