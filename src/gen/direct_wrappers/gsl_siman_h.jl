...
> distance. The simulated annealing steps are generated using the random
> number generator r and the function take\_step.
>
> The starting configuration of the system should be given by x0\_p. The
> routine offers two modes for updating configurations, a fixed-size
> mode and a variable-size mode. In the fixed-size mode the
> configuration is stored as a single block of memory of size
> element\_size. Copies of this configuration are created, copied and
> destroyed internally using the standard library functions malloc,
> memcpy and free. The function pointers copyfunc, copy\_constructor and
> destructor should be null pointers in fixed-size mode. In the
> variable-size mode the functions copyfunc, copy\_constructor and
> destructor are used to create, copy and destroy configurations
> internally. The variable element\_size should be zero in the
> variable-size mode.
>
...
> algorithm.
>
> On exit the best result achieved during the search is placed in x0\_p.
> If the annealing process has been successful this should be a good
> approximation to the optimal point in the space.
>
> If the function pointer print\_position is not null, a debugging log
> will be printed to `stdout`{.sourceCode} with the following columns:
>
>     #-iter  #-evals  temperature  position  energy  best_energy
>
> and the output of the function print\_position itself. If
> print\_position is null then no information is printed.
