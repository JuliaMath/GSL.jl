...
#
#   Returns: Cint
function multiset_fwrite(stream::Ref{Cvoid}, c::Ref{gsl_multiset})
    errno = ccall( (:gsl_multiset_fwrite, libgsl), Cint, (Ref{Cvoid},
...
#
#   Returns: Cint
function multiset_fread(stream::Ref{Cvoid})
    c = Ref{gsl_multiset}()
    errno = ccall( (:gsl_multiset_fread, libgsl), Cint, (Ref{Cvoid},
        Ref{gsl_multiset}), stream, c )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return c[]
end
...
#
#   Returns: Cint
function multiset_fprintf(stream::Ref{Cvoid}, c::Ref{gsl_multiset})
    format = Ref{Cchar}()
    errno = ccall( (:gsl_multiset_fprintf, libgsl), Cint, (Ref{Cvoid},
...
#
#   Returns: Cint
function multiset_fscanf(stream::Ref{Cvoid})
    c = Ref{gsl_multiset}()
    errno = ccall( (:gsl_multiset_fscanf, libgsl), Cint, (Ref{Cvoid},
        Ref{gsl_multiset}), stream, c )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return c[]
end
