...
# 
#   Returns: Cint
function histogram2d_fwrite(stream::Ref{Nothing}, h::Ref{gsl_histogram2d})
    errno = ccall( (:gsl_histogram2d_fwrite, libgsl), Cint, (Ref{Nothing},
...
# 
#   Returns: Cint
function histogram2d_fread(stream::Ref{Nothing})
    h = Ref{gsl_histogram2d}()
    errno = ccall( (:gsl_histogram2d_fread, libgsl), Cint, (Ref{Nothing},
        Ref{gsl_histogram2d}), stream, h )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return h[]
end
...
# 
#   Returns: Cint
function histogram2d_fprintf(stream::Ref{Nothing}, h::Ref{gsl_histogram2d})
    range_format = Ref{Cchar}()
    bin_format = Ref{Cchar}()
    errno = ccall( (:gsl_histogram2d_fprintf, libgsl), Cint, (Ref{Nothing},
...
# 
#   Returns: Cint
function histogram2d_fscanf(stream::Ref{Nothing})
    h = Ref{gsl_histogram2d}()
    errno = ccall( (:gsl_histogram2d_fscanf, libgsl), Cint, (Ref{Nothing},
        Ref{gsl_histogram2d}), stream, h )
    if errno!= 0 throw(GSL_ERROR(errno)) end
    return h[]
end
