...
function ran_dir_2d(r::Ref{gsl_rng})
    x = Ref{Cdouble}()
    y = Ref{Cdouble}()
    ccall( (:gsl_ran_dir_2d, libgsl), Nothing, (Ref{gsl_rng}, Ref{Cdouble},
        Ref{Cdouble}), r, x, y )
    return x[], y[]
end
...
function ran_dir_2d_trig_method(r::Ref{gsl_rng})
    x = Ref{Cdouble}()
    y = Ref{Cdouble}()
    ccall( (:gsl_ran_dir_2d_trig_method, libgsl), Nothing, (Ref{gsl_rng},
...
function ran_dir_3d(r::Ref{gsl_rng})
    ...
    y = Ref{Cdouble}()
    z = Ref{Cdouble}()
    ccall( (:gsl_ran_dir_3d, libgsl), Nothing, (Ref{gsl_rng}, Ref{Cdouble},
...
function ran_dir_nd(r::Ref{gsl_rng}, n::Integer)
    x = Ref{Cdouble}()
    ccall( (:gsl_ran_dir_nd, libgsl), Nothing, (Ref{gsl_rng}, Csize_t,
        Ref{Cdouble}), r, n, x )
    return x[]
end
